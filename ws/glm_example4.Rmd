...
```{r}
loyn.glm2 <- glm(abund ~ sl_dist + sl_ldist + sl_area + fgraze + 
                   salt + syr_isol, data = loyn,
                family = gaussian(link = "identity"))
loyn.glm2 %>% ggemmeans(~sl_dist) %>% plot(add=T)
loyn.glm2 %>% ggemmeans(~sl_ldist) %>% plot(add=T)
loyn.glm2 %>% ggemmeans(~salt) %>% plot(add=T)
loyn.glm2 %>% ggemmeans(~syr_isol) %>% plot(add=T)
loyn.glm2 %>% ggemmeans(~sl_area + fgraze) %>% plot(add=T)
```



```{r}
plot_model(loyn.glm2, type = 'eff', show.data = T, dot.size = 0.5) %>%
  plot_grid
```
...
```{r}
plot(allEffects(loyn.glm2, residuals = T), type = "response")
```
...
loyn.glm1 <- update(loyn.glm, . ~ 
                      scale(l_dist, scale=F) * scale(l_ldist, scale=F))
loyn.glm2 <- update(loyn.glm, . ~ 
                      scale(l_area, scale=F) * fgraze)
loyn.glm3 <- update(loyn.glm, . ~ 
                      scale(l_area, scale=F) * fgraze * scale(yr_isol, scale=F))
