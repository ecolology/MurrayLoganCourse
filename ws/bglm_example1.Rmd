...
fert_list = with(fert, list(fert = seq(min(fert), max(fert), len=100)))
newdata = emmeans(fert_rstanarm3, ~fert, at=fert_list) %>% as.data.frame
head(newdata)

ggplot(newdata, aes(y=emmean, x=fert)) + 
...
scale_x_continuous('fert') +
theme_classic()

## spaghetti plot
newdata = emmeans(fert_rstanarm3, ~fert, at=fert_list) %>%
  gather_emmeans_draws()
newdata %>% head
ggplot(newdata,  aes(y=.value,  x=fert)) +
...
fert_list = with(fert, list(fert = seq(min(fert), max(fert), len=100)))
newdata = emmeans(fert_brm3, ~fert, at=fert_list) %>% as.data.frame
head(newdata)
...
## spaghetti plot
newdata = emmeans(fert_brm3, ~fert, at=fert_list) %>%
  gather_emmeans_draws()
newdata %>% head
ggplot(newdata,  aes(y=.value,  x=fert)) +
  geom_line(aes(group=.draw),  alpha=0.01) +
