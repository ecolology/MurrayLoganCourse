...
```{r}
owls_glmm <- glmmTMB(ncalls ~ food + sex + offset(log(brood_size)) + (1|nest),
        data = owls, REML=T)
```
...
plot_model(owls_glmm,  type='eff',  terms=c('FoodTreatment', 'SexParent'))
```
...
plot(allEffects(owls_glmm), multiline=TRUE, ci.style='bars')
```
...
ggpredict(owls_glmm,  terms=c('FoodTreatment', 'SexParent')) %>% plot
```
...
as.numeric(off)
ggemmeans(owls_glmm,  ~FoodTreatment+SexParent, offset=log(off$Mean)) %>% plot
```
...
ggemmeans(owls_glmm,  ~FoodTreatment+SexParent, offset=0) %>% plot
```
...
newdata = emmeans(owls_glmm, ~FoodTreatment+SexParent, at=owls.grid,
...
newdata=tidy(owls.glmmTMB3, effects='fixed', conf.int=TRUE,  exponentiate=TRUE) %>%
  ...
  bind_rows(
    tidy(owls_glmm, effects='fixed', conf.int=TRUE,  exponentiate=TRUE) %>%
...
newdata = emmeans(owls.glmmTMB3, ~FoodTreatment+SexParent, offset=0, at=owls.grid, type='response') %>%
  ...
  bind_rows(
    emmeans(owls_glmm, ~FoodTreatment+SexParent, offset=0, at=owls.grid, type='response') %>%
